...
export function Hero() {
  ...
  return (
    <header id="hero" className="relative isolate flex min-h-[100svh] overflow-hidden">
      <MatrixRain
        opacity={0.16}
        className="pointer-events-none absolute inset-0 -z-20 h-full w-full [mask-image:radial-gradient(ellipse_80%_70%_at_50%_30%,black_20%,transparent_85%)]"
      />
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 80% 60% at 70% 50%, rgba(124,58,237,0.14) 0%, transparent 60%), radial-gradient(ellipse 50% 50% at 15% 80%, rgba(0,255,102,0.08) 0%, transparent 60%)",
        }}
      />
      ...
      <div className="mx-auto flex w-full max-w-[1200px] flex-col-reverse items-center gap-10 px-[5%] pb-16 pt-[calc(72px+2.5rem)] md:flex-row md:items-center md:justify-between md:gap-16 md:pt-[calc(72px+3rem)]">
        <div className="max-w-[640px] text-center md:text-left">
          <motion.p
            ...
            animate="visible"
            variants={fadeUp}
            className="mb-3 flex items-center justify-center gap-2 font-mono text-sm text-green md:justify-start"
          >
            <span className="text-ink-3">guest@dem</span>
          ...
          >
            <DecryptedText text={profile.firstName} />
            <br />
            <DecryptedText
              text={profile.lastName}
              delay={0.15}
              className="text-glow bg-gradient-to-br from-green to-cyan bg-clip-text text-transparent"
            />
          </motion.h1>
          ...
          >
            <MagneticButton
              ...
              download
              className="inline-flex items-center gap-2 rounded-full bg-gradient-to-br from-green to-[#0af5c8] px-8 py-3.5 text-sm font-bold text-bg shadow-[0_4px_24px_rgba(0,255,102,0.3)] transition-shadow hover:shadow-[0_8px_32px_rgba(0,255,102,0.45)]"
            >
              <Download size={16} />
              Download Resume
            </MagneticButton>
      ...
      >
        <span>[ scroll ]</span>
        <span className="h-10 w-px bg-gradient-to-b from-green/40 to-transparent" />
      </motion.div>
    </header>
  );
}
