  ...
  return (
    <div
      ...
      className={cn(
        ...
        glow && "border-green/[0.12] bg-gradient-to-br from-green/[0.04] to-violet/[0.04]"
      )}
    >
      <div
        ...
        style={{
          background:
            "radial-gradient(circle at var(--mx, 50%) var(--my, 50%), rgba(0,255,102,0.08), transparent 60%)",
        }}
      />
      <div className="relative">
        ...
        <div className="flex flex-wrap gap-2">
          {group.items.map((item) => (
            <span
              key={item}
              className="rounded-full border border-border bg-white/[0.04] px-3 py-1.5 text-[0.78rem] text-ink-2 transition-all duration-200 hover:-translate-y-0.5 hover:border-green/25 hover:bg-green/10 hover:text-green"
            >
              {item}
            </span>
          ))}
